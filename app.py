...
def obtener_cliente_ia():
    """Crea el cliente de Anthropic con la API key cargada por el usuario."""
    if anthropic is None:
        st.error("La librería 'anthropic' no está instalada. Agregala a requirements.txt.")
        return None
    clave = st.session_state.api_key or st.secrets.get("ANTHROPIC_API_KEY", "")
    if not clave:
        st.warning("Ingresá tu API key de Anthropic en la barra lateral para usar esta función.")
        return None
    try:
...
with st.sidebar:
    ...
    st.session_state.api_key = st.text_input(
        ...
        help="Necesaria para el Asistente de Consultas y el Generador de Escritos.",
    )
    st.markdown("---")
